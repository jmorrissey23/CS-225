...
class Graph {
    public:
        struct Node {
            Node(std::string course) : data(course){};
            std::set<Node*> adjList;
            std::string data;
            int color = -1;
            std::set<std::string> students; //maps all the classes to the students
        };
        Graph(V2D roster);
        std::vector<std::string> incidentEdges(Node* v); 
        bool areAdjacent(Node* v1, Node* v2); 
        void insertEdge(Node* v1, Node* v2);
        std::vector<Node*> nodeList_; 
        void resetColors();
    private:
        std::map<std::string, Node*> courses; //maps the course name to its respective vertex
};