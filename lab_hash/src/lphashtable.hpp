...
void LPHashTable<K, V>::remove(K const& key)
{
    ...
    if (index != -1) { //if the key is in the table
...
int LPHashTable<K, V>::findIndex(const K& key) const
{
    ...
    size_t counter = 0;
    while (counter < size) {
        if (table[hash] != NULL && table[hash] -> first == key) {
            return hash;
        }
        hash = (hash + 1) % size;
        counter++;
...
void LPHashTable<K, V>::resizeTable()
{
     ...
     * @hint Use findPrime()!
     */
    int prime = findPrime(size*2);
    std::pair<K, V>** resized_table = new std::pair<K, V>*[prime];
    delete[] should_probe;
    ...
    for (size_t i = 0; i < size; i++) {
        if (table[i] != NULL) {
            int hash = hashes::hash(table[i]->first, prime);
            while (resized_table[hash] != NULL) {
                hash = (hash + 1) % prime;
            } 
            resized_table[hash] = table[i];
            should_probe[hash] = true;
        }
    }
    delete[] table;
