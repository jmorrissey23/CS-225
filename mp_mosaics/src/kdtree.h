...
class KDTree
{
  ...
  private:
    ...
    Point<Dim> QuickSelect(vector<Point<Dim>>& newPoints, int left, int right, int k, int dimension);
    int Partition(vector<Point<Dim>>& newPoints, int left, int right, int pivotIndex, int dimension);
    typename KDTree<Dim>::KDTreeNode* RecursiveConstructor(vector<Point<Dim>>& newPoints, int left, int right, int dimension);

    void copy(const KDTree<Dim>& rhs);
    void clear(KDTreeNode* subroot);

    Point<Dim> ForwardNeighbor(const Point<Dim>& query, KDTreeNode* current, int dimension, std::stack<KDTreeNode*> stack, std::set<KDTreeNode*>& seen) const;
    Point<Dim> BackwardsNeighbor(const Point<Dim>& query, int dimension, KDTreeNode* best, std::stack<KDTreeNode*> stack, std::set<KDTreeNode*>& seen) const;

    double Radius(Point<Dim> point1, Point<Dim> point2) const;
    KDTreeNode* RecurseOther(const Point<Dim>& query, int dimension, KDTreeNode* best, KDTreeNode* root, std::set<KDTreeNode*>& seen) const;

    KDTreeNode* findNearestNeighbor(const Point<Dim>& query, int dim, KDTreeNode* current, std::set<KDTreeNode*>& seen) const;
