...
<<<<<<< HEAD
    for (size_t i = 0; i < frequencies.size(); i++) {
        TreeNode* node = new TreeNode(frequencies.at(i));
    ...
    while (!(singleQueue.empty()) || mergeQueue.size() != 1) {
        if (mergeQueue.empty()) { // first iteration
            TreeNode* left = singleQueue.front();
            singleQueue.pop();
            TreeNode* right = singleQueue.front();
            singleQueue.pop();
            TreeNode* parent = new TreeNode(left-> freq.getFrequency() + right -> freq.getFrequency());
            parent -> left = left;
            parent -> right = right;
            mergeQueue.push(parent);
        } else {
            if (singleQueue.empty()) { // only left with merged 
                TreeNode* left = mergeQueue.front();
                mergeQueue.pop();
                TreeNode* right = mergeQueue.front();
                mergeQueue.pop();
                TreeNode* parent = new TreeNode(left-> freq.getFrequency() + right -> freq.getFrequency());
                parent -> left = left;
                parent -> right = right;
