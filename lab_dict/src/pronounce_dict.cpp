...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    std::string first;
    std::string second;
    first.resize(word1.size());
    second.resize(word2.size());
    std::transform(word1.begin(), word1.end(), first.begin(),  ::toupper);
    std::transform(word2.begin(), word2.end(), second.begin(), ::toupper);
    
    if (dict.count(first) == 0 || dict.count(second) == 0) { // if either one of the words isn't in the dictionary
        return false;
    } 
    
    std::vector<std::string> translation1 = dict.at(first);
    std::vector<std::string> translation2 = dict.at(second);

    for (size_t i = 0; i < translation1.size(); i++) {
        if (translation1[i] != translation2[i]) {
            return false;
        }
    }
    return true;
